...
static const char *gettype(const char *buf)
{
	...
	char gif[] = { 0x47, 0x49, 0x46, 0x38, 0x61, 0x37, 0x39, };
	char jpg[] = { 0xff, 0xd8, 0xff, 0xe0, 0xe1, 0xdb, };
	char xpm[] = { '/', '*', ' ', 'X', 'P', 'M', ' ', '*', '/', };
	...
	if (match(buf, png, sizeof(png))) {
		...
		return "image/jpg";
	} else if (match(buf, xpm, sizeof(xpm))) {
		return "image/xpm";
	} else if (match(buf, "<!DOCTYPE html>", slen("<!DOCTYPE html>"))) {
		return "text/html";
	} else {
...
static int directlink(const char *req)
{
	if (match(req, "/files/", slen("/files/")))
		return 1;
	else if (match(req, "/tmp/", slen("/tmp/")))
		return 1;
	else if (match(req, "/styles/", slen("/styles/")))
		return 1;
	else if (match(req, "/fonts/", slen("/fonts/")))
		return 1;
	return 0;
}
...
static int parseget(struct context *ctx, char *buf, int16_t buflen)
{
	...
	*tmp = '\0'; /* not interested in the rest */
	...
	if (directlink(req)) {
		uint8_t len; /* do not expect more than that */
		len = tmp - req;
		if (len > sizeof(ctx->buf))
			len = sizeof(ctx->buf);
		memcpy(ctx->buf, req + 1, len);
		ctx->path = ctx->buf;
		dd("direct link %s\n", ctx->path);
		return 0;
		     ...
		     struct request *reqend)
{
	...
	while (reqptr <= reqend) {
		str = reqptr->str;
		len = umin32(reqptr->len, slen("Content-Type:"));
...
static int pushfile(struct context *ctx, struct iovec *iov)
{
	...
	eval(pushv(ctx->sd, iov, HTTP_HDR_MAX) < 0, ret = -1; goto out);

	lseek(fd, 0, SEEK_SET);
	ret = sendfile(ctx->sd, fd, 0, st.st_size);
	dd("sd %d, fd %d, size %ld, ret %d\n", ctx->sd, fd, st.st_size, ret);
	if (ret != st.st_size)
		ret = -1;
	else
		ret = 0;
...
static int parsereq(struct context *ctx)
{
	...
	cnt = pullstr(ctx->sd, buf, slen(buf), req, ARRAY_SIZE(req));
	if (cnt < 1)
		return -1; /* no data */
	reqptr = req;
	reqend = reqptr + cnt;
	while (reqptr < reqend) {
		...
		len = reqptr->len;
		ii("req %s, len %d\n", str, len);
		if (match(str, "GET", slen("GET"))) {
			return parseget(ctx, str, len);
		} else if (match(str, "POST", slen("POST"))) {
